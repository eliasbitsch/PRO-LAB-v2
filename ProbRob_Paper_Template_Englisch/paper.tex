...
All four research filters are recursive Bayes filters~\cite{Thrun2005}: each maintains a belief over the robot pose and alternates a \emph{predict} step (advance the belief through a motion model) with a \emph{correct} step (reweight it by a measurement likelihood). They differ in how the belief is represented and how non-linearity is handled. The \textbf{KF} assumes a Gaussian belief and \emph{linear} models, giving a closed-form optimal update; to keep both the motion and the measurement models linear it cannot use the trigonometric unicycle, so it instead carries a six-dimensional constant-velocity state and learns the velocities passively. The \textbf{EKF} keeps the Gaussian belief but linearises the non-linear unicycle and range--bearing models about the current mean via their Jacobians $\mathbf{G}_t,\mathbf{H}_t$. The \textbf{EKF-LF} is the same Gaussian machinery with the landmark correction replaced by a lidar likelihood-field update. The \textbf{PF} drops the Gaussian assumption entirely: it represents the belief by $N$ weighted pose samples (particles), so it can be \emph{multimodal} and, in principle, localise globally and recover from kidnapping --- at $O(N)$ cost per step. Table~\ref{tab:filters} summarises the state vector, control input, and models of each filter.
...
\hline
\textbf{Filter} & \textbf{State $\mathbf{x}$} & \textbf{Input $\mathbf{u}$} & \textbf{Correction} \\ \hline
KF     & $(x,y,\theta,\dot x,\dot y,\dot\theta)$ & --- & landmark $(x,y)$, IMU \\ \hline
EKF    & $(x,y,\theta)$ & $(v,\omega)$ \texttt{/odom}     & landmark range--bearing, IMU \\ \hline
EKF-LF & $(x,y,\theta)$ & $(v,\omega)$ \texttt{/cmd\_vel} & scan likelihood field, IMU \\ \hline
PF     & $\{(x,y,\theta)^{[k]}\}$ & $(v,\omega)$ \texttt{/cmd\_vel} & scan likelihood field, IMU \\ \hline
AMCL   & $\{(x,y,\theta)^{[k]}\}$ & \texttt{/odom} (TF) & scan likelihood field \\ \hline
\end{tabular}}
\caption{Per-filter state vector, control input, and measurement correction. The KF carries world-frame velocities $(\dot x,\dot y,\dot\theta)$ in its state (constant-velocity model, no control input); the others use the three-dimensional unicycle pose with a velocity/odometry control. PF and AMCL hold $N$ pose samples rather than a single Gaussian.}
\label{tab:filters}
\end{table}
...
\end{equation}
with covariance $\mathbf{P}_t=\mathbf{G}_t\mathbf{P}_{t-1}\mathbf{G}_t^\top+\mathbf{R}_t$, where $\mathbf{G}_t$ is the motion Jacobian. Here $\Delta s$ and $\Delta\theta$ are the translation and rotation increments read from wheel odometry (\texttt{/odom}). The linear KF instead carries the six-dimensional state $\mathbf{x}=(x,y,\theta,\dot x,\dot y,\dot\theta)^\top$ and propagates it with a constant-velocity model $\mathbf{x}_t=\mathbf{A}\,\mathbf{x}_{t-1}$, $\mathbf{A}=\big[\begin{smallmatrix}\mathbf{I} & \Delta t\,\mathbf{I}\\ \mathbf{0} & \mathbf{I}\end{smallmatrix}\big]$, which is exactly linear and therefore takes no control input: the velocities $(\dot x,\dot y,\dot\theta)$ are not commanded but inferred from the sequence of position and yaw measurements, with the process noise on the velocity block absorbing the unmodelled accelerations. This keeps the KF a genuine linear filter (any trigonometric unicycle term would force a Jacobian, i.e.\ an EKF) and serves as a deliberately simple reference. The \emph{landmark} filters (KF, EKF) correct on a range--bearing observation of a known map landmark $\mathbf{m}_i=(m_{i,x},m_{i,y})$,
\begin{equation}
...
We define eleven scenarios that share the same scripted trajectory. Seven probe the \emph{initial belief}: \texttt{correct\_init} (nominal); \texttt{offset\_1m} and \texttt{offset\_5m} (translated start); \texttt{wrong\_yaw\_pi2} ($90^\circ$ heading error); \texttt{overconfident\_wrong} (wrong start with a tight covariance); \texttt{underconfident} (correct start with an inflated covariance); and \texttt{kidnapped} (two unannounced teleports during the run). Four further scenarios cover the mandatory \emph{noise} sweep, varying the filters' noise tuning while initializing at the truth so that the noise is the only changing factor: \texttt{q\_low}/\texttt{q\_high} scale every process-noise term by $\tfrac{1}{10}$/$10\times$ its default, and \texttt{r\_low}/\texttt{r\_high} do the same for every measurement-noise term, with \texttt{correct\_init} serving as the default midpoint. For each filter we report the position error RMSE$_{xy}$, the convergence rate (the fraction of seeds whose final position error falls below $0.20$\,m), the filter consistency via the normalized estimation error squared (NEES, over the 3-DOF pose, ideally $\approx 3$), the per-tick wall-clock runtime, and the asymptotic complexity. Every scenario is repeated for ten random seeds that perturb the sampled initial pose and the particle-filter randomness; we report the mean and one standard deviation over seeds. The full sweep comprises $11\times10=110$ runs.
...
\subsection{Noise sensitivity: process (Q) and measurement (R)}
The mandatory noise sweep isolates the effect of the filters' noise tuning by initializing at the truth and varying only Q or R. Because the start is correct, the position error stays low throughout ($0.08$--$0.27$\,m), with one telling exception: under \texttt{r\_low} the scan EKF-LF over-trusts an aliased lidar and degrades to $0.63$\,m. The tuning therefore acts almost entirely on \emph{confidence}, not nominal accuracy, which is exactly what the NEES consistency metric exposes (Fig.~\ref{fig:noise}, Table~\ref{tab:nees}). On both axes the mean NEES falls monotonically as the noise grows: too little noise makes a filter \emph{over-confident} (covariance too tight, NEES~$\gg 3$), too much makes it \emph{conservative} (NEES~$\ll 3$). The Gaussian landmark filters KF and EKF are the best calibrated, sitting near the consistent line at the default and bracketing it across the sweep. The sample- and scan-based filters are chronically over-confident: EKF-LF and PF already exceed NEES~$=10$ at the default, and under \texttt{r\_low} the PF's particle cloud collapses to a near-singular covariance (NEES~$\sim\!10^{8}$), the textbook symptom of particle impoverishment when the measurement model is trusted too far. AMCL is conservative throughout (NEES~$<1$), reflecting its deliberately inflated motion model. The practical reading matches the task's framing: Q sets how much the filter trusts its own motion model and R how much it trusts its sensors, and only a balanced choice yields a statistically consistent, trustworthy covariance.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.96\textwidth]{images/noise_sensitivity.png}
    \caption{Filter consistency (mean NEES over the 3-DOF pose, log scale, ten seeds) versus process noise Q (left) and measurement noise R (right), with \texttt{correct\_init} as the default midpoint. NEES decreases monotonically with added noise and crosses the consistent line (dof~$=3$): low noise yields over-confident filters, high noise conservative ones. Bars above the cap are labelled with their value.}
    \label{fig:noise}
\end{figure*}
...
\begin{tabular}{|l|c|c|c|c|c|}
\hline
 & \multicolumn{3}{c|}{\textbf{Process noise Q}} & \multicolumn{2}{c|}{\textbf{Meas.\ noise R}} \\ \cline{2-6}
\textbf{Filter} & low & default & high & low & high \\ \hline
KF      & $2.1$  & $1.0$  & $0.36$ & $3.6$            & $0.25$ \\ \hline
EKF     & $1.8$  & $0.57$ & $0.13$ & $1.4$            & $0.20$ \\ \hline
EKF-LF  & $13.6$ & $4.0$  & $1.9$  & $102.6$          & $0.88$ \\ \hline
PF      & $12.5$ & $11.9$ & $4.1$  & $\sim\!10^{8}$   & $0.92$ \\ \hline
AMCL    & $1.2$  & $0.48$ & $0.06$ & $0.77$           & $0.06$ \\ \hline
\end{tabular}}
\caption{Mean NEES (consistency over the 3-DOF pose; consistent $\approx 3$) under the Q and R sweep, averaged over ten seeds. \texttt{correct\_init} is the shared default. Values fall as noise rises on both axes; KF/EKF stay closest to consistent, while EKF-LF and PF are markedly over-confident at low noise.}
\label{tab:nees}
\end{table}

\subsection{Threats to validity}
The study is in simulation, so odometry is more benign than on hardware, which flatters dead reckoning in the nominal case. The forward-facing camera limits landmark visibility, a realistic but scenario-dependent constraint. Finally, fiducials require instrumenting the environment; our claim is not that tags are universally preferable but that, where identifiable landmarks are available, they convert the recovery problem from intractable to easy in exactly the aliased settings that defeat scan matching. The scan-based filters retain real advantages we do not dispute: they need no environment instrumentation, work in any mapped space, and---unlike the unimodal Gaussian landmark filters---can represent global, multi-modal uncertainty at all; indeed the KF and EKF localize globally only because the tag identity hands them the correspondence. The result is therefore a trade-off, not a verdict: scan matching is universal but aliasing-vulnerable under a wrong start, whereas identifiable landmarks buy autonomous recovery at the cost of instrumenting the environment.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Summary and Outlook}

We compared five localization filters across eleven scenarios on a TurtleBot~4 in a perceptually symmetric warehouse, with the four research filters implemented from first principles and landmark identity measured honestly from camera-read AprilTags. Across ten seeds per scenario, filters that observe identifiable landmarks recovered from large offsets, wrong heading, miscalibrated covariance, and double kidnapping, whereas scan-based filters, including the Nav2 AMCL baseline, were defeated by perceptual aliasing and diverged by up to $17$\,m. The advantage was specific to bad initialization and disappeared under nominal conditions, where all filters localized to within $0.15$\,m. The dedicated process- and measurement-noise sweep confirmed the expected confidence trade-off: NEES fell monotonically from over-confident at low noise to conservative at high noise, with the Gaussian landmark filters staying best calibrated and the sample-based filters over-confident. The decisive factor for recovery is therefore the identifiability of the observation model, not the sophistication of the estimator.
